class Solution:
    def maxSubArray(self, nums):
        ...
        :rtype: int
        """
        low, high, sum = self.find_maximum_subarray(nums, 0, len(nums)-1)
        return sum

    def find_max_crossing_subarray(self, nums, low, mid, high):
        left_sum = float("-inf")
        max_left = 0
        sum = 0
    ...
    def find_maximum_subarray(self, nums, low, high):
        if high == low:
            return low, high, nums[low]
        else:
            mid = (low+high)//2
            left_low, left_high, left_sum = \
                self.find_maximum_subarray(nums, low, mid)
            right_low, right_high, right_sum = \
                self.find_maximum_subarray(nums, mid+1, high)
            cross_low, cross_high, cross_sum = \
                self.find_max_crossing_subarray(nums, low, mid, high)
